...
import { BarChart3, Check, Clock3, Gauge, Headphones, LockKeyhole, MessageSquareText, Package, Plus, Save, Search, ShoppingBag, Trash2, UsersRound, X } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  return (
    <main className="admin-shell">
      <aside className="admin-sidebar"><div className="admin-brand"><span>మ</span><div><strong>Mana Mart</strong><small>Shop console</small></div></div><nav><a className="active"><Package /> Inventory</a><a><ShoppingBag /> Orders</a><a><MessageSquareText /> Conversations</a><a><BarChart3 /> Usage</a></nav><div className="sidebar-footer">Telugu · Hindi · English<br/><span>Supabase live database</span></div></aside>
      <section className="admin-main">
        <header><div><p className="eyebrow">SHOP OPERATIONS</p><h1>Mana Mart</h1></div><div className="live-pill"><i /> Live inventory</div></header>
        ...
        {modelConfig && <section className="model-console"><div className="model-console-head"><div><p className="eyebrow">SARVAM VOICE STACK</p><h2><Gauge /> Voice intelligence</h2><span>Sarvam handles speech recognition, conversation intelligence, and speech output with one API key.</span></div></div><div className="model-order">{modelConfig.order.map((id, index) => { const model = modelConfig.models.find((item) => item.id === id); if (!model) return null; return <article className="primary-model" key={id}><b>{index + 1}</b><div><strong>{model.name}</strong><span>{model.note}</span></div><em className={model.available ? "ready" : "missing"}>{model.available ? <><Check /> Key ready</> : "Key missing"}</em></article>; })}</div></section>}
